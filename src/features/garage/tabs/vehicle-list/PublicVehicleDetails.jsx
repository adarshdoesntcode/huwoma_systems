    ...
    content = (
      <>
        <header className="flex sticky top-0 h-14 items-center gap-4 z-50 bg-slate-100/50 backdrop-filter backdrop-blur-lg px-4 lg:h-[60px] lg:px-6">
          <div className="relative max-w-[69rem] mx-auto flex-1">
        ...
        <div className="flex flex-col h-full max-w-6xl mx-auto duration-300 fade-in animate-in slide-in-from-bottom-4">
          <div className="flex-1 p-4 bg-white rounded-lg sm:p-6">
            ...
            <div className="grid grid-cols-12 gap-4 mt-8 sm:gap-6">
              ...
              <div className="col-span-12 lg:col-span-6">
                <div className="space-y-3">
                  ...
                  {/* {vehicle.transactions?.length > 0 && vehicle.status === "Sold" && (
                  <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col gap-1">
                            <p
                              ...
                              onClick={() =>
                                navigate(
                                  `/garage/customers/${vehicle.transactions[vehicle.transactions.length - 1].buyer._id}`
                                )
                              }
                            >
                              {vehicle.transactions[vehicle.transactions.length - 1].buyer.name}
                              <ArrowUpRight className="w-4 h-4 ml-1 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                            </p>

                            <p className="flex items-center text-xs">
                              {vehicle.transactions[vehicle.transactions.length - 1].buyer.contactNumber}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge className="text-sm text-green-600 border-green-100 bg-green-50">
                            {formatCurrency(vehicle.transactions[vehicle.transactions.length - 1].sellingPrice)}
                          </Badge>
                        </div>
                      </div>
                    </div>
                    ...
                    vehicle.status === "Sold" && (
                      <div className="flex items-center justify-between gap-2">
                        {/* <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                          <Calendar className="w-3 h-3 text-gray-500" />
                          Sold on{" "}
                          {formatDate(vehicle.transactions[vehicle.transactions.length - 1].transactionTime)}
                        </div>
                        <div className="text-[10px] text-muted-foreground">
                          {getDaysDifference(
                            vehicle.transactions[vehicle.transactions.length - 1].transactionTime,
                            new Date()
                          )}
                          d ago
                        </div> */}
